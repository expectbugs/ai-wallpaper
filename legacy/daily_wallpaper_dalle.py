#!/usr/bin/env python3
"""
AI-Powered Daily Wallpaper Generator - DALL-E Edition
Phase 1: Basic prompt generation with deepseek-r1:14b
Phase 2: Image generation with DALL-E 3
...
def generate_prompt_with_deepseek(history):
    """Generate a unique, creative image prompt using deepseek-r1:14b with themed guidance for DALL-E"""
    log("Generating creative prompt with deepseek-r1:14b for DALL-E...")
    ...
    # DeepSeek instruction optimized for DALL-E (shorter, clearer prompts)
    deepseek_instruction = f"""
Generate a single, vividly descriptive image prompt for an amazing image, optimized for DALL-E 3.
...
- Use clear, concrete visual descriptions that DALL-E can interpret.
...
- Specify the artistic style and color palette from the theme.
...
- Focus on photorealistic, ultra-detailed rendering.
...
def generate_image(prompt):
    """Generate image using DALL-E 3 API - NO FALLBACKS"""
    log("Starting image generation with DALL-E 3...")
    ...
    try:
        ...
        log(f"Image generated successfully, downloading from URL...")
    except Exception as e:
    ...
    try:
        ...
        log(f"Image downloaded and saved: {filename}")
    except Exception as e:
        log(f"ERROR: Failed to download/save image: {e}")
        sys.exit(1)
...
def process_image_to_4k(image_path):
    """Process DALL-E image to 4K with crop, upscale, and downsample"""
    ...
    if image.size != (1792, 1024):
        log(f"WARNING: Unexpected dimensions {image.size}, expected (1792, 1024)")
    
    # Crop height from 1024 to 1008 (remove 8px top, 8px bottom)
    image_cropped = image.crop((0, 8, 1792, 1016))  # 1016 = 1024 - 8
    ...
    temp_cropped = "/tmp/dalle_cropped.png"
    ...
    # Run Real-ESRGAN
    temp_output_dir = "/tmp/dalle_upscaled"
    ...
    if image_upscaled.size != (7168, 4032):
        log(f"WARNING: Unexpected upscaled size {image_upscaled.size}, expected (7168, 4032)")
...
def full_generation():
    ...
    log("=== STARTING DAILY WALLPAPER GENERATION (DALL-E EDITION) ===")
    log("Pipeline: Prompt → DALL-E → 4K Processing → Wallpaper")
    ...
    log("Step 5/8: Generating image with DALL-E 3...")
    image_path = generate_image(prompt)
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="AI-Powered Daily Wallpaper Generator - DALL-E Edition")
    ...
    if args.test_prompt:
        ...
        log("Usage: daily_wallpaper_dalle.py [--run-now | --test-prompt | --test-image | --test-wallpaper]")
