...
Phase 1: Direct intelligent prompt generation with GPT-4o
...
def generate_image_with_context():
    """Generate image directly with GPT-4o using theme and context - NO INTERMEDIATE PROMPT"""
    log("Starting direct image generation with GPT-4o using theme and context...")
    
    # Get context and theme information
    ...
    # Create filename based on theme and date
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    theme_excerpt = sanitize_filename(f"{theme_result['theme']['name']}_{theme_result['category']}")
    filename = f"{timestamp}_{theme_excerpt}.png"
    filepath = os.path.join(IMAGES_DIR, filename)
    
    log(f"Image will be saved to: {filepath}")
    
    # Create comprehensive description for GPT-4o
    image_description = f"""Create a stunning, photorealistic, hyperrealistic, extremely detailed and high quality wallpaper image.

Theme: {theme_result['theme']['name']} from {theme_result['category']} category
{theme_result['instruction']}

Context: It's {context['day_of_week']}, {context['month']} {context['date'].split('-')[2]}, {context['year']} in {context['season']}
Weather: {context['weather']['condition']}, {context['weather']['temperature']}, Wind: {context['weather']['wind']}, Mood: {context['weather']['mood']}

Requirements:
- Create a complete scene with foreground, midground, and background elements
- Include specific lighting and atmospheric details that match the weather and time
- Use the artistic style and color palette from the theme
- Incorporate the weather conditions and seasonal context into the visual elements
- Focus on photorealistic, ultra-detailed rendering suitable as a 4K desktop wallpaper
- Be creative and surprising while following the theme guidance"""
    
    log(f"Sending comprehensive context to GPT-4o: {image_description[:200]}...")
    
    # GPT-4o direct image generation via Responses API (highest quality settings)
    log("Calling GPT-4o for direct image generation with highest quality settings...")
    ...
    for attempt in range(max_attempts):
        try:
            response = openai_client.responses.create(
                model="gpt-4o",  # Using gpt-4o for highest quality results
                input=image_description,
                tools=[{
                    "type": "image_generation",
                    "size": "1536x1024",      # Landscape format for wallpaper
            ...
            log(f"API call attempt {attempt + 1} failed: {e}")
            if attempt == max_attempts - 1:
                log("ERROR: All API call attempts failed")
                sys.exit(1)
    ...
    try:
        ...
        if not image_generation_calls:
            log("ERROR: No image generation call found in response")
            log(f"Response output types: {[output.type for output in response.output]}")
            sys.exit(1)
        
        image_call = image_generation_calls[0]
        log(f"Image generation call status: {image_call.status}")
        
        if hasattr(image_call, 'revised_prompt') and image_call.revised_prompt:
            log(f"GPT-4o revised description: {image_call.revised_prompt[:100]}...")
        
        if image_call.status != "completed":
            log(f"ERROR: Image generation failed with status: {image_call.status}")
            sys.exit(1)
        
        image_base64 = image_call.result
        log("Successfully extracted base64 image data from response")
        
    except Exception as e:
        log(f"ERROR: Failed to extract image data from response: {e}")
        log(f"Response structure: {type(response)}")
        sys.exit(1)
...
def test_theme_context():
    """Test function to show theme and context selection"""
    log("=== STARTING THEME AND CONTEXT TEST ===")
    
    # Get context and theme information
    context = get_context_info()
    theme_result = get_random_theme_with_weather(context['weather'])
    
    # Display the selected theme and context
    log(f"Selected theme: {theme_result['theme']['name']} from category {theme_result['category']}")
    log(f"Theme instruction: {theme_result['instruction'][:200]}...")
    log(f"Context: {context['day_of_week']}, {context['month']} {context['date'].split('-')[2]}, {context['year']} in {context['season']}")
    log(f"Weather: {context['weather']['condition']}, {context['weather']['temperature']}, Wind: {context['weather']['wind']}, Mood: {context['weather']['mood']}")
    
    log("=== THEME AND CONTEXT TEST COMPLETE ===")
    
    return context, theme_result

def test_image_generation():
    """Test function for image generation with theme and context"""
    log("=== STARTING IMAGE GENERATION TEST ===")
    
    # Generate image directly with theme and context
    image_path = generate_image_with_context()
...
def full_generation():
    """Complete wallpaper generation from theme/context to desktop"""
    log("=== STARTING DAILY WALLPAPER GENERATION (GPT-IMAGE-1 EDITION) ===")
    log("Pipeline: Theme + Context → GPT-4o + GPT-Image-1 → 4K Processing → Wallpaper")
    
    start_time = datetime.now()
    
    # Step 1: Generate image directly with theme and context
    log("Step 1/4: Generating image with GPT-4o using theme and context...")
    image_path = generate_image_with_context()
    log(f"Image saved: {os.path.basename(image_path)}")
    
    # Step 2: Process to 4K
    log("Step 2/4: Processing image to 4K...")
    final_image_path = process_image_to_4k(image_path)
    
    # Step 3: Set wallpaper
    log("Step 3/4: Setting desktop wallpaper...")
    set_wallpaper(final_image_path)
    
    # Step 4: Verify wallpaper
    log("Step 4/4: Verifying wallpaper...")
...
def main():
    ...
    parser.add_argument("--test-theme", action="store_true", help="Test theme and context selection only")
    parser.add_argument("--test-image", action="store_true", help="Test image generation with theme and context")
    parser.add_argument("--test-wallpaper", action="store_true", help="Test wallpaper setting only")
    parser.add_argument("--run-now", action="store_true", help="Run full generation: theme + context → image → wallpaper")
    ...
    if args.test_theme:
        ...
        log("Usage: daily_wallpaper_gpt.py [--run-now | --test-theme | --test-image | --test-wallpaper]")
        log("  --run-now: Generate new wallpaper (full pipeline)")
        log("  --test-theme: Test theme and context selection only")
        log("  --test-image: Test image generation with theme and context")
